...
type UploadQueueStatus = "Example" | "Queued" | "Blocked" | "Failed";
...
const requestRows: RequestRowViewModel[] = [
  ...
  { id: "REQ-1027", request: "Website Refresh Imagery", requester: "Avery Scott", due: "May 14, 2024", status: "Needs Review", priority: "Medium", owner: "Morgan Riley", usage: "Web" },
  ...
  { id: "REQ-1041", request: "Q2 Paid Social Ad Assets", requester: "Jordan Lee", due: "May 15, 2024", status: "Review Hold", priority: "High", owner: "Taylor Morgan", usage: "Social" },
...
function UploadStatusChip({ status }: { status: UploadQueueStatus }) {
  const tone = status === "Failed" || status === "Blocked" ? "danger" : status === "Queued" ? "draft" : "review";
      ...
      <div className="proto-collection-detail">
        <main>
          ...
          {tab === "Assets" ? <div className="proto-collection-grid">
            {assets.slice(0, 11).map((asset) => <AssetCard key={asset.id} asset={asset} selected={false} active={false} onSelect={() => toast.message("Collection asset selection is read-only here. Open Library to create governed collection drafts.")} onInspect={() => window.location.assign(routeWithRole(`/assets/${asset.id}`, role))} />)}
...
export function PrototypeUploadIntake() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(payload.message || payload.error || (response.ok ? "Submitted for review. Not public. Source/original remains restricted." : "Upload blocked. Intake fields remain on screen so you can fix and retry."));
    } catch (error) {
      setMessage(`Upload blocked. Intake fields remain on screen so you can fix and retry. ${error instanceof Error ? error.message : "Network request failed."}`);
    } finally {
      setSubmitting(false);
    }
  }
  ...
  const queueProgress = 0;
  const queueLabel = files.length ? `${files.length} file${files.length === 1 ? "" : "s"} staged for intake submission` : "Reference intake examples only; no files selected or uploaded";
  const queueRows: Array<{ file: UploadListItem | File; status: UploadQueueStatus; progress: number }> = queueFiles.slice(0, 8).map((file, index) => {
    const status: UploadQueueStatus = files.length ? "Queued" : file.size > 100_000_000 ? "Blocked" : "Example";
    const progress = 0;
  ...
  return (
    <form className="proto-page proto-upload-page" onSubmit={submit}>
      <PageHeader title="Upload / Intake" subtitle="Stage intake packets. Browser files stay local unless private durable upload storage is proven.">
        ...
        <PillButton type="submit" tone="primary" disabled={!canSend || submitting}><Upload size={16} />{submitting ? "Submitting..." : canSend ? "Submit intake" : "Contributor access required"} <ChevronDown size={14} /></PillButton>
      </PageHeader>
      <div className="proto-upload-workspace">
        <main>
          <section className="proto-upload-drop-panel">
            ...
            <div className="proto-upload-settings"><h2>Intake settings</h2><label>Duplicates<select defaultValue="flag"><option value="flag">Flag exact matches for review</option></select></label><label>Color profile<select defaultValue="note"><option value="note">Record conversion note</option></select></label></div>
            <div className="proto-upload-settings"><h2>Review options</h2>{["Request preview generation", "Request metadata extraction", "Suggest AI tags"].map((item) => <label className="proto-toggle-row" key={item}>{item}<input type="checkbox" defaultChecked /></label>)}</div>
          </section>
          ...
          <section className="proto-panel proto-upload-queue">
            ...
            {queueRows.map(({ file, status, progress }, index) => <div className="proto-upload-row" key={`${file.name}-${index}`}><span>{assets[index] ? <AssetImage asset={assets[index]} /> : <FileText size={18} />}</span><strong>{file.name}</strong><small>{formatBytes(file.size)}</small><UploadStatusChip status={status} /><div className="proto-upload-progress"><ProgressMeter value={progress} /><small>{files.length ? "Pending submit" : "Preview only"}</small></div><button type="button" aria-label={files.length ? `Remove ${file.name}` : `Example file ${file.name}`} onClick={() => files.length ? setFiles((current) => current.filter((_, fileIndex) => fileIndex !== index)) : toast.message("Select real files to edit the intake queue.")}>{files.length ? <X size={14} /> : <FileText size={14} />}</button></div>)}
            <footer><span>{queueFiles.length} files listed</span><span>{files.length ? "Pending intake submit" : "No active upload"}</span><strong>{formatBytes(queueFiles.reduce((sum, file) => sum + file.size, 0))}</strong></footer>
          </section>
          ...
          <div className="proto-upload-bottom">
            <section className="proto-panel"><h2>Validation</h2><p><StatusChip label="Description review needed" tone="review" /></p><p><StatusChip label="Derivative work not proven" tone="draft" /></p><p><StatusChip label="Duplicates checked after intake" tone="review" /></p><PillButton onClick={() => toast.message("Validation details remain local to the intake packet until submitted.")}>View details</PillButton></section>
            ...
            <section className="proto-panel"><h2>Batch summary</h2><dl className="proto-dl"><div><dt>Total files</dt><dd>{queueFiles.length}</dd></div><div><dt>Uploaded</dt><dd>0</dd></div><div><dt>Staged</dt><dd>{files.length ? queueRows.filter((row) => row.status === "Queued").length : 0}</dd></div><div><dt>Blocked</dt><dd>{queueRows.filter((row) => row.status === "Blocked").length}</dd></div><div><dt>Total size</dt><dd>{formatBytes(queueFiles.reduce((sum, file) => sum + file.size, 0))}</dd></div></dl></section>
          </div>
        </main>
      ...
      {message ? <p className={`proto-gate-note ${/blocked|failed|error/i.test(message) ? "is-warning" : "is-success"}`}>{message}</p> : null}
    </form>
  );
}
...
function PrototypeReviewQueue() {
  ...
  const bucketStats = [
    ["Needs Review", rows.filter((row) => assetEnterpriseStatus(row.asset) === "Needs Review").length || rows.length, "Assets submitted and awaiting your approval"],
    ["Needs Evidence", rows.filter((row) => /missing|unclear|unknown|required/i.test(`${row.asset.rightsStatus || ""} ${row.asset.consentStatus || ""} ${row.asset.rightsNotes || ""}`)).length, "Proof or source documentation requested"],
    ["Approved Internal", rows.filter((row) => /internal/i.test(row.asset.usageScope || "")).length, "Approved internally, pending next step"],
    ["Blocked", rows.filter((row) => assetEnterpriseStatus(row.asset) === "Restricted").length, "Action required before approval can proceed"],
    ["Expiring Soon", rows.filter((row) => row.asset.rightsExpirationDate).length, "Assets expiring within 30 days"]
  ] as const;
  ...
  return (
    <section className="proto-page proto-review-queue-page">
      ...
      <div className="proto-review-workbench">
        <aside className="proto-review-buckets">{bucketStats.map(([label, count, detail], index) => <button type="button" className={index === 0 ? "is-active" : ""} key={label} onClick={() => toast.message(`${label} bucket selected. Queue data remains role-gated and read-only until a review action is submitted.`)}><span /><strong>{label}<em>{count.toLocaleString()}</em></strong><small>{detail}</small></button>)}<section><h3>Your review snapshot</h3><div><strong>{rows.length}</strong><span>Visible</span></div><div><strong>{bucketStats[1][1]}</strong><span>Needs evidence</span></div><div><strong>{bucketStats[3][1]}</strong><span>Blocked</span></div><footer>Writeback mode <strong>Queued first</strong></footer></section></aside>
        <main className="proto-panel proto-review-table"><header className="proto-panel-head"><h2>{queueLoading && !rows.length ? "Loading review queue" : `${rows.length} assets`}</h2><button type="button" onClick={() => toast.message("Review queue sorted by due date in this beta.")}>Sort by: Due date (soonest) <ChevronDown size={14} /></button></header><div className="proto-data-table"><div className="proto-table-head"><span><input type="checkbox" /> Asset</span><span>Tags</span><span>Status</span><span>Due date</span><span>Assignee</span><span>Review stage</span><span>Priority</span><span /></div>{rows.length ? rows.map((row) => <Link href={routeWithRole(`/review/${row.id}`, role)} className="proto-table-row" key={row.id}><span><input type="checkbox" /><span className="proto-row-thumb"><AssetImage asset={row.asset} /></span><strong>{displayTitle(row.asset)}<small>{assetMeta(row.asset)}</small><small>{assetRecordRef(row.asset)}</small></strong></span><span className="proto-review-tag-cell">{(row.asset.tags || row.asset.tjcTerms || []).slice(0, 3).map((tag) => <em key={tag}>{tag}</em>)}</span><span><StatusChip asset={row.asset} /></span><span>{row.due}</span><span><span className="proto-mini-avatar" />{row.assignee}</span><span>{row.stage}</span><span><StatusChip label={row.priority} tone={row.priority === "High" ? "danger" : row.priority === "Medium" ? "review" : "approved"} /></span><span><MoreHorizontal size={15} /></span></Link>) : queueLoading ? Array.from({ length: 5 }).map((_, index) => <div className="proto-table-row proto-skeleton-row" key={index}><span><span className="proto-row-thumb" /><strong>Loading review asset<small>ResourceSpace/export queue</small></strong></span><span /><span /><span /><span /><span /><span /><span /></div>) : <div className="proto-empty-table">No review assets match current filters. Clear filters or open Library needs-review view.</div>}</div><footer className="proto-pagination">{rows.length ? `1–${rows.length} of ${rows.length} items` : queueLoading ? "Loading items" : "0 items"} <span><button type="button" onClick={() => toast.message("Page 1 active.")}>1</button><button type="button" onClick={() => toast.message("Next review page preview. Queue state did not change.")}><ChevronRight size={15} /></button></span></footer></main>
      </div>
    </section>
  );
}
    ...
    <section className="proto-page proto-review-detail-page">
      ...
      <div className="proto-review-detail-grid">
        <main>
          ...
          <section className="proto-comparison-panel"><div><header><strong>Version 1</strong><span>Prior version unavailable in current export; showing safe current preview for comparison frame.</span></header><AssetImage asset={selected} variant="detail" /></div><div><header><strong>Version 2</strong><span>Current safe preview derivative</span></header><AssetImage asset={selected} variant="detail" /></div><button type="button" className="proto-split-handle" onClick={() => setMessage("Comparison mode stays visual only; source/original versions are not opened.")}><ChevronLeft size={15} /><ChevronRight size={15} /></button><footer><span><Search size={15} /> − + 100%</span><span>Side by side · Overlay · Swipe</span><span>Full screen</span></footer></section>
...
export function PrototypeRequestsPage() {
  ...
  const { role } = useDemoRole();
  const requestAssets = useAssetsSearch({ role, sort: "Approved first", limit: 8 });
  ...
  return (
    <section className="proto-page proto-requests-page">
      <div className="proto-requests-grid">
        <main>
          <PageHeader title="Requests" count={`${requestRows.length} examples`} subtitle="Preview request examples only. Durable request workflow storage is not proven." search={query} onSearch={setQuery}>
          ...
          <div className="proto-filter-pills">{["Inbox", "All requests", "My requests", "Flagged"].map((item, index) => <button type="button" className={tab === item ? "is-active" : ""} key={item} onClick={() => setTab(item)}>{item} <span>{[requestRows.filter((row) => row.status !== "Approved").length, requestRows.length, requestRows.filter((row) => row.owner === "Taylor Morgan").length, requestRows.filter((row) => row.priority === "High").length][index]}</span></button>)}</div>
          <section className="proto-data-table proto-requests-table"><div className="proto-table-head"><span><input type="checkbox" /> Request</span><span>Requester</span><span>Due date</span><span>Status</span><span>Priority</span><span>Owner</span><span>Usage</span><span /></div>{rows.map((row) => <button type="button" className={`proto-table-row ${row.id === selected.id ? "is-active" : ""}`} key={row.id} onClick={() => setSelectedId(row.id)}><span><input type="checkbox" checked={row.id === selected.id} readOnly /><strong>{row.request}<small>· Spring Campaign 2024</small></strong></span><span><span className="proto-mini-avatar" />{row.requester}<small>Marketing</small></span><span>{row.due}</span><span><StatusChip label={row.status === "Pending Approval" ? "Pending" : row.status} tone={row.status === "New" ? "draft" : row.status === "Review Hold" ? "danger" : "review"} /></span><span><StatusChip label={row.priority} tone={row.priority === "High" ? "danger" : row.priority === "Medium" ? "review" : "approved"} /></span><span><span className="proto-mini-avatar" />{row.owner}</span><span>{row.usage}</span><span><MoreHorizontal size={15} /></span></button>)}</section>
        </main>
        {selected ? <aside className="proto-request-panel"><div className="proto-inspector-nav"><ChevronLeft size={15} /><span>1 of {rows.length}</span><ChevronRight size={15} /><X size={15} /></div><header><h2>{selected.request}</h2><StatusChip label={selected.status} tone={selected.status === "Review Hold" ? "danger" : "review"} /><p>Example request from {selected.requester}. No durable request record was changed.</p></header><SegmentedTabs tabs={["Details", "Comments", "Activity"]} active={panelTab} onChange={setPanelTab} />{panelTab === "Details" ? <><section><h3>Request overview</h3><dl className="proto-dl"><div><dt>Due date</dt><dd>{selected.due}</dd></div><div><dt>Priority</dt><dd>{selected.priority}</dd></div><div><dt>Requested usage</dt><dd>{selected.usage}</dd></div></dl><p>Request details are preview examples. Media team must verify asset rights before use or delivery.</p></section><section><h3>Approval state</h3><div className="proto-stepper"><span className="is-done">Requested</span><span className="is-active">Review needed</span><span>Decision pending</span><span>Delivery blocked</span></div></section><section><h3>Requested files</h3><div className="proto-request-files">{(requestAssets.data?.assets || []).slice(0, 6).map((asset) => <span key={asset.id}><AssetImage asset={asset} /></span>)}</div><PillButton onClick={() => setMessage("Requested files use role-safe media records. Request fulfillment stays blocked until durable workflow storage exists.")}>View all files</PillButton></section><section><h3>Fulfillment</h3><label className="proto-field"><span>Owner</span><input readOnly value="Taylor Morgan" /></label><label className="proto-field"><span>Due date</span><input readOnly value={selected.due} /></label><label className="proto-field"><span>Priority</span><input readOnly value={selected.priority} /></label></section></> : <section><h3>{panelTab}</h3><p>{panelTab === "Comments" ? "Comments will persist after durable request workflow storage is connected. No fake request update is recorded in local beta." : "Activity is read-only for local beta request examples."}</p></section>}<footer><PillButton onClick={() => setMessage("Request status update is blocked until durable request workflow storage is connected. No status changed.")}>Update status</PillButton><PillButton tone="primary" onClick={() => setMessage("Fulfillment is disabled in local beta; no fake completion was recorded.")}>Request fulfillment</PillButton></footer>{message ? <p className="proto-gate-note">{message}</p> : null}</aside> : <aside className="proto-request-panel"><p>No requests match this filter.</p></aside>}
      </div>
    </section>
  );
}
  ...
  return (
    <section className="proto-page proto-admin-meta-page">
      ...
      <div className="proto-admin-meta-grid">
        ...
        <section className="proto-panel proto-reports-preview"><header className="proto-panel-head"><div><h2>Reports Preview</h2><p>Readiness facts only. Usage analytics stay unclaimed until durable event logging exists.</p></div><PillButton onClick={() => toast.message("Reports preview uses current readiness/search data only. No live usage success claimed.")}>Current snapshot <ChevronDown size={14} /></PillButton></header><div className="proto-mini-metrics"><MetricCard label="Assets" value={(readiness.data?.assetCount || 0).toLocaleString()} trend="catalog snapshot" /><MetricCard label="Portal ready" value={(readiness.data?.metrics?.portalReady || 0).toLocaleString()} trend="not public launch" /><MetricCard label="Needs Review" value={(readiness.data?.metrics?.needsReview || 0).toLocaleString()} trend="review queue" /><MetricCard label="Durable storage" value={runtimeStore?.state || "Unknown"} trend="proof required" /></div></section>
