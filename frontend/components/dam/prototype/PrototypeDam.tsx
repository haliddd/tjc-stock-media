    ...
    <aside className="proto-inspector" aria-label="Asset inspector">
      ...
      {tab === "details" ? (
        ...
        <div className="proto-detail-stack">
          ...
          <label className="proto-comment-box"><input placeholder="Add a comment..." /><button type="button"><Send size={15} /></button></label>
        </div>
      )}
      ...
      <section className="proto-library-workspace">
        ...
        {results.loading ? <div className="proto-loading">Loading library...</div> : results.error ? <div className="proto-error">{results.error}</div> : (
          <div className={`proto-asset-grid ${viewMode === "list" ? "is-list" : ""}`}>
            {assets.map((asset) => <AssetCard key={asset.id} asset={asset} selected={selected.has(asset.id)} active={activeAsset?.id === asset.id} onSelect={() => toggle(asset.id)} onInspect={() => setActiveId(asset.id)} />)}
          </div>
        )}
      </section>
  ...
  return (
    <section className="proto-page proto-collections-index">
      ...
      <div className="proto-filter-pills">
        ...
        <button type="button">Sort by: Last updated <ChevronDown size={14} /></button>
        <IconButton label="Grid"><LayoutGrid size={16} /></IconButton>
        <IconButton label="List"><List size={16} /></IconButton>
      </div>
      ...
      <section className="proto-panel">
        ...
        <footer className="proto-pagination">1–{rows.length} of {collections.length || 48} collections <span><button>1</button><button>2</button><button>3</button><button>4</button><ChevronRight size={15} /></span></footer>
      </section>
    </section>
  );
}
    ...
    <section className="proto-page proto-collection-detail-page">
      <div className="proto-collection-detail">
        <main>
          ...
          <div className="proto-collection-toolbar"><label className="proto-checkbox-label"><input type="checkbox" /> <span>{assets.length || selected?.count || 12} assets</span></label><PillButton><SlidersHorizontal size={16} />Filters</PillButton><div className="proto-toolbar-spacer" /><button className="proto-sort" type="button">Sort by: Newest <ChevronDown size={14} /></button><IconButton label="Grid"><LayoutGrid size={16} /></IconButton><IconButton label="List"><List size={16} /></IconButton></div>
...
export function PrototypeUploadIntake() {
  ...
  const sampleQueueFiles: UploadListItem[] = [
  ...
  const queueFiles = files.length ? files : sampleQueueFiles;
  const queueProgress = files.length ? 0 : 67;
  const queueLabel = files.length ? `${files.length} file${files.length === 1 ? "" : "s"} ready to submit` : "Sample queue shown until files/source link are selected";
  ...
  return (
    <form className="proto-page proto-upload-page" onSubmit={submit}>
      <PageHeader title="Upload / Intake" subtitle="Batch upload assets and assign metadata.">
        <PillButton>Save as draft</PillButton>
      ...
      <div className="proto-upload-workspace">
        <main>
          ...
          <section className="proto-panel proto-upload-queue">
            <header className="proto-panel-head"><div><h2>Upload queue</h2><p>{queueLabel}</p></div><progress value={queueProgress} max={100} /><span>{queueProgress}%</span><PillButton onClick={() => toast.message("Pause applies to active uploads; selected files are only staged locally until Start upload.")}><Pause size={14} />Pause all</PillButton><PillButton onClick={() => setFiles([])}><X size={14} />Cancel all</PillButton></header>
            {queueFiles.slice(0, 8).map((file, index) => <div className="proto-upload-row" key={`${file.name}-${index}`}><span>{assets[index] ? <AssetImage asset={assets[index]} /> : <FileText size={18} />}</span><strong>{file.name}</strong><small>{formatBytes(file.size)}</small><em>{files.length ? "Staged" : index < 2 ? "Completed sample" : index < 4 ? "Uploading sample" : "Queued sample"}</em><progress value={files.length ? 0 : index < 2 ? 100 : index === 2 ? 74 : index === 3 ? 48 : 0} max={100} /><button type="button" onClick={() => files.length ? setFiles((current) => current.filter((_, fileIndex) => fileIndex !== index)) : toast.message("Sample queue item. Select real files to edit queue.")}>{files.length ? <X size={14} /> : index < 4 ? <Pause size={14} /> : <X size={14} />}</button></div>)}
            <footer><span>{queueFiles.length} files in total</span><span>Estimated time remaining: 2 min</span><strong>2.1 GB</strong></footer>
          </section>
          <section className="proto-panel"><header className="proto-panel-head"><h2>Uploaded (8)</h2></header><ThumbnailStrip assets={assets} /></section>
          <div className="proto-upload-bottom">
            <section className="proto-panel"><h2>Validation</h2><p><StatusChip label="2 files missing description" tone="review" /></p><p><StatusChip label="1 file color profile will be converted" tone="draft" /></p><p><StatusChip label="No duplicate files detected" tone="approved" /></p><PillButton>View details</PillButton></section>
            <section className="proto-panel"><h2>AI tagging suggestions</h2><div className="proto-tag-row">{["outdoor", "nature", "mountain", "lake", "travel", "portrait", "architecture", "modern", "minimal"].map((tag) => <span key={tag}>{tag} <CheckCircle2 size={11} /></span>)}</div><footer><PillButton>Review all</PillButton><PillButton tone="primary">Apply all</PillButton></footer></section>
        ...
        <aside className="proto-upload-side">
          <section className="proto-panel"><header className="proto-panel-head"><h2>Assign metadata</h2><ChevronDown size={14} /></header>{(["collection", "brand", "usageRights", "credit", "description", "sourceLink", "tags"] as const).map((key) => <label key={key} className="proto-field"><span>{key === "usageRights" ? "Usage rights" : key === "sourceLink" ? "Source link" : key[0].toUpperCase() + key.slice(1)}</span><input value={form[key]} onChange={(event) => setForm((current) => ({ ...current, [key]: event.target.value }))} /></label>)}<h3>Rights & Usage</h3><label className="proto-field"><span>License</span><select defaultValue="standard"><option value="standard">Standard License</option></select></label><label className="proto-field"><span>Restrictions</span><input defaultValue="No public use until review" /></label><label className="proto-field"><span>Expiry date</span><input type="date" value={form.eventDate} onChange={(event) => setForm((current) => ({ ...current, eventDate: event.target.value }))} /></label></section>
          <section className="proto-panel"><header className="proto-panel-head"><h2>Recent uploads</h2><button type="button">View all</button></header>{assets.slice(0, 5).map((asset) => <p className="proto-mini-asset" key={asset.id}><span><AssetImage asset={asset} /></span><strong>{displayTitle(asset)}<small>{asset.importDate || "May 14, 2024"}</small></strong></p>)}</section>
        </aside>
      </div>
      {message ? <p className="proto-gate-note">{message}</p> : null}
    </form>
...
function PrototypeReviewQueue() {
  ...
  const rows = reviewRows(queue.data?.assets || [])
    .filter((row) => displayTitle(row.asset).toLowerCase().includes(query.toLowerCase()))
    .filter((row) => priority === "All" || row.priority === priority);

  if (!canReview(role)) {
    return <section className="proto-page"><div className="proto-panel"><h1>Review Queue</h1><p>Reviewer access required.</p></div></section>;
  }

  return (
    <section className="proto-page proto-review-queue-page">
      <PageHeader title="Review Queue" count={`${rows.length || 27}`} subtitle="Assets awaiting your review and approval">
        <PillButton>Saved views <ChevronDown size={14} /></PillButton><PillButton><SlidersHorizontal size={16} />Filters <span className="proto-count">2</span></PillButton><PillButton><Settings size={16} />Queue settings</PillButton><PillButton tone="primary">Bulk actions <ChevronDown size={14} /></PillButton>
      </PageHeader>
      ...
      <div className="proto-review-workbench">
        <aside className="proto-review-buckets">{[["Needs Review", 14, "Assets submitted and awaiting your approval"], ["Needs Evidence", 6, "Proof or source documentation requested"], ["Approved Internal", 32, "Approved internally, pending next step"], ["Blocked", 3, "Action required before approval can proceed"], ["Expiring Soon", 8, "Assets expiring within 30 days"]].map(([label, count, detail], index) => <button type="button" className={index === 0 ? "is-active" : ""} key={label}><span /><strong>{label}<em>{count}</em></strong><small>{detail}</small></button>)}<section><h3>Your review snapshot</h3><div><strong>27</strong><span>Pending</span></div><div><strong>18</strong><span>Approved</span></div><div><strong>4</strong><span>Returned</span></div><footer>Avg. time to review <strong>1.3 days</strong></footer></section></aside>
        <main className="proto-panel proto-review-table"><header className="proto-panel-head"><h2>{rows.length || 14} assets</h2><button type="button">Sort by: Due date (soonest) <ChevronDown size={14} /></button></header><div className="proto-data-table"><div className="proto-table-head"><span><input type="checkbox" /> Asset</span><span>Status</span><span>Due date</span><span>Assignee</span><span>Review stage</span><span>Priority</span><span /></div>{rows.map((row) => <Link href={routeWithRole(`/review/${row.id}`, role)} className="proto-table-row" key={row.id}><span><input type="checkbox" /><span className="proto-row-thumb"><AssetImage asset={row.asset} /></span><strong>{displayTitle(row.asset)}<small>{assetMeta(row.asset)}</small><small>{(row.asset.tags || []).slice(0, 3).join(" · ")}</small></strong></span><span><StatusChip asset={row.asset} /></span><span>{row.due}</span><span><span className="proto-mini-avatar" />{row.assignee}</span><span>{row.stage}</span><span><StatusChip label={row.priority} tone={row.priority === "High" ? "danger" : row.priority === "Medium" ? "review" : "approved"} /></span><span><MoreHorizontal size={15} /></span></Link>)}</div><footer className="proto-pagination">1–{rows.length} of {rows.length} items <span><button>1</button><ChevronRight size={15} /></span></footer></main>
      </div>
    </section>
  );
}
...
function PrototypeReviewDetail({ requestId }: { requestId: string }) {
  const { role } = useDemoRole();
  const queue = useReviewQueue(role, "pending");
  const selected = queue.data?.assets.find((asset) => asset.id === requestId) || queue.data?.assets[0];
  ...
  if (!canReview(role)) return <section className="proto-page"><div className="proto-panel"><h1>Review Detail</h1><p>Reviewer access required.</p></div></section>;
  ...
  return (
    <section className="proto-page proto-review-detail-page">
      ...
      <div className="proto-review-detail-grid">
        <main>
          <div className="proto-version-bar"><Link href={routeWithRole("/library", role)}>Back to Library</Link><StatusChip asset={selected} /><button type="button">Version 2 (Current) <ChevronDown size={14} /></button><span /><IconButton label="Previous"><ChevronLeft size={16} /></IconButton><IconButton label="Next"><ChevronRight size={16} /></IconButton></div>
          <section className="proto-comparison-panel"><div><header><strong>Version 1</strong><span>May 10, 2024 · 6.1 MB</span></header><AssetImage asset={selected} variant="detail" /></div><div><header><strong>Version 2</strong><span>May 14, 2024 · 7.2 MB</span></header><AssetImage asset={selected} variant="detail" /></div><button type="button" className="proto-split-handle"><ChevronLeft size={15} /><ChevronRight size={15} /></button><footer><span><Search size={15} /> − + 100%</span><span>Side by side · Overlay · Swipe</span><span>Full screen</span></footer></section>
          <div className="proto-review-detail-lower"><section className="proto-panel"><header className="proto-panel-head"><h2>Comments (4)</h2><button type="button">Newest <ChevronDown size={14} /></button></header><label className="proto-comment-box"><input placeholder="Add a comment..." value={comment} onChange={(event) => setComment(event.target.value)} /><button type="button"><Send size={15} /></button></label>{["Love the composition. Can we brighten the shadows slightly?", "Updated the shadows and warmed the tone slightly.", "Much better, approved.", "Please confirm model release is attached."].map((text, index) => <p className="proto-comment-item" key={text}><span className="proto-mini-avatar" /><strong>{index % 2 ? "Jordan Lee" : "Taylor Morgan"}<small>May 14, 2024 · {index + 9}:15 AM</small></strong><span>{text}</span></p>)}</section><section className="proto-panel"><h2>Change requests (1)</h2><p><strong>CR-2024-17</strong> Confirm model release form is attached to this asset.</p><h2>Evidence checklist</h2>{reviewChecklistItems.map((item) => <label key={item.field} className="proto-check-row"><input type="checkbox" checked={Boolean(checklist[item.field])} onChange={() => setChecklist((current) => ({ ...current, [item.field]: !current[item.field] }))} />{item.label}</label>)}</section></div>
        </main>
        <aside className="proto-review-side">
          <InspectorPanel asset={selected} index={0} total={1} />
          <section className="proto-panel proto-decision-card"><header className="proto-panel-head"><h2>Review decision</h2><StatusChip label="Required" tone="review" /></header>{[["Approve", "Asset is approved and available for use.", "approved"], ["Request changes", "Return to uploader with feedback.", "review"], ["Restrict", "Limit usage or access permissions.", "draft"], ["Block", "Prevent usage and hide from users.", "danger"]].map(([label, detail, tone]) => <button type="button" key={label} onClick={() => void decide(label === "Approve" ? "Approve Public" : "Request More Info")}><StatusChip label={label} tone={tone} /><span>{detail}</span><ChevronRight size={15} /></button>)}<label className="proto-field"><span>Reviewer notes</span><textarea placeholder="Add notes about your decision..." value={comment} onChange={(event) => setComment(event.target.value)} /></label><p>Your decision will be recorded in the activity log.</p></section>
        </aside>
      </div>
      {message ? <p className="proto-gate-note">{message}</p> : null}
    </section>
...
export function PrototypeUsersGroupsPage() {
  ...
  if (!canAdmin(role)) return <section className="proto-page"><div className="proto-panel"><h1>Users & Groups</h1><p>DAM Admin access required.</p></div></section>;
      ...
      <div className="proto-users-grid">
        <main>
          <PageHeader title="Users & Groups" subtitle="Manage users, groups, roles, and access." search={query} onSearch={setQuery}>
            ...
            <PillButton tone="primary" onClick={() => setMessage("Invite user is disabled in local beta. SSO/group sync must be configured before sending invites.")}><UserPlus size={16} />Invite user</PillButton>
          </PageHeader>
        ...
        <aside className="proto-user-permissions">
          ...
          <section><h3>{panelTab}</h3><p><strong>Effective role</strong> <StatusChip label={selected.role} tone={selected.role === "DAM Admin" ? "danger" : "review"} /></p><h4>Permission summary</h4>{["Manage users & groups", "Manage roles & policies", "Manage library content", "Manage workflow", "Manage settings & integrations", "View reports & analytics"].map((item) => <p className="proto-permission-row" key={item}>{item}<span>{selected.role === "Viewer" ? "Scoped" : "Full access"} <CheckCircle2 size={14} /></span></p>)}<PillButton onClick={() => setMessage("Role permissions are read-only in beta; RBAC enforcement remains active.")}>View role permissions</PillButton><h4>Access scope</h4><div className="proto-scope-card"><Folder size={18} /><strong>All collections<small>Global access</small></strong><PillButton onClick={() => setMessage("Scope edits disabled until SSO/group sync is connected.")}>Change scope</PillButton></div><dl className="proto-dl"><div><dt>Joined</dt><dd>Jan 12, 2024</dd></div><div><dt>Last active</dt><dd>{selected.lastActive}</dd></div></dl><PillButton tone="danger" onClick={() => setMessage("Deactivate user disabled in local beta. No identity changes were made.")}>Deactivate user</PillButton>{message ? <p className="proto-gate-note">{message}</p> : null}</section>
        </aside>
      </div>
    </section>
  );
...
function PrototypeAdminMetadataBrand({ initialModule }: { initialModule?: string }) {
  ...
  if (!canAdmin(role)) return <section className="proto-page"><div className="proto-panel"><h1>Admin / Metadata & Brand</h1><p>DAM Admin access required.</p></div></section>;
  ...
  return (
    <section className="proto-page proto-admin-meta-page">
      ...
      <div className="proto-admin-meta-grid">
        ...
        <section className="proto-panel"><header className="proto-panel-head"><div><h2>Taxonomies</h2><p>Manage controlled vocabularies and hierarchical taxonomies.</p></div><PillButton>Manage all</PillButton></header>{[["Asset Type", "12 terms · 2 levels"], ["Usage Rights", "8 terms"], ["Location", "245 terms · 3 levels"], ["Audience", "6 terms"], ["Campaign", "14 terms · 2 levels"]].map(([label, detail], index) => <p className="proto-tax-row" key={label}><span className={`is-${index}`}><Tag size={14} /></span><strong>{label}<small>{detail}</small></strong><MoreHorizontal size={15} /></p>)}</section>
        <aside className="proto-brand-kit-panel"><section className="proto-panel"><header className="proto-panel-head"><div><h2>Brand Kit</h2><p>Manage logos, colors, typography and brand assets.</p></div></header><label className="proto-field"><span>Brand</span><select defaultValue="tjc"><option value="tjc">TJC Media</option></select></label><h3>Logos</h3><div className="proto-logo-grid"><span>TJC</span><span>TJC</span><span>◎</span></div><PillButton><Plus size={14} />Add logo</PillButton><h3>Colors</h3><div className="proto-color-swatches">{["#000", "#242424", "#e2cbb6", "#ece8df", "#f7f6f4"].map((color) => <span key={color} style={{ background: color }} />)}<small>+8</small></div><h3>Typography</h3><p className="proto-type-row"><strong>Ag</strong><span>Source Sans<br />Regular · Medium · Semibold</span><PillButton>Manage fonts</PillButton></p><h3>Brand assets</h3><ThumbnailStrip assets={brandAssets.data?.assets || []} limit={4} /><PillButton>View all assets</PillButton></section><section className="proto-panel"><h2>Inheritance</h2>{["Default metadata", "Brand inheritance", "Collection overrides"].map((item) => <p className="proto-settings-row" key={item}><Lock size={14} /><strong>{item}<small>Inherited from Global</small></strong><ChevronRight size={14} /></p>)}</section></aside>
        <section className="proto-panel"><header className="proto-panel-head"><div><h2>Validation Rules</h2><p>Enforce data quality and consistency across your metadata.</p></div><PillButton><Plus size={14} />Add rule</PillButton></header>{["Required fields", "Controlled vocabulary", "File type constraints", "Unique filename"].map((item, index) => <p className="proto-settings-row" key={item}><ShieldCheck size={14} /><strong>{item}<small>{index === 3 ? "Prevent duplicate filenames in the system." : "Ensure critical standards are always enforced."}</small></strong><StatusChip label={index === 3 ? "Inactive" : "Active"} tone={index === 3 ? "draft" : "approved"} /></p>)}</section>
        <section className="proto-panel"><header className="proto-panel-head"><div><h2>Controlled Vocabularies</h2><p>Manage term lists used across metadata fields.</p></div><PillButton><Plus size={14} />Add vocabulary</PillButton></header>{[["Asset Type", "12 terms"], ["Usage Rights", "8 terms"], ["Location", "245 terms"], ["Audience", "6 terms"]].map(([label, detail]) => <p className="proto-settings-row" key={label}><Circle size={12} /><strong>{label}<small>{detail}</small></strong><MoreHorizontal size={15} /></p>)}</section>
        <section className="proto-panel proto-reports-preview"><header className="proto-panel-head"><div><h2>Reports Preview</h2><p>Real-time insights into your asset library.</p></div><PillButton>Last 30 days <ChevronDown size={14} /></PillButton></header><div className="proto-mini-metrics"><MetricCard label="Assets" value={((readiness.data?.metrics?.portalReady || 0) + (readiness.data?.metrics?.needsReview || 0) || 12456).toLocaleString()} trend="↑ 12.5%" /><MetricCard label="Downloads" value="8,974" trend="↑ 8.1%" /><MetricCard label="Views" value="25,431" trend="↑ 15.3%" /><MetricCard label="Approvals" value="1,245" trend="↓ 4.2%" /></div></section>
        <section className="proto-panel"><h2>Global Settings</h2>{["Metadata settings", "Integrations", "Security & Access", "Notifications"].map((item) => <p className="proto-settings-row" key={item}><Settings size={14} /><strong>{item}<small>Configure platform-wide preferences and policies.</small></strong><PillButton>Manage</PillButton></p>)}</section>
      </div>
    </section>
  );
}
