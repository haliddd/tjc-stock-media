...
type UploadQueueStatus = "Completed" | "Uploading" | "Queued" | "Failed";
...
function UploadStatusChip({ status }: { status: UploadQueueStatus }) {
  const tone = status === "Completed" ? "approved" : status === "Failed" ? "danger" : status === "Uploading" ? "review" : "draft";
  ...
  return (
    <form className="proto-page proto-upload-page" onSubmit={submit}>
      <PageHeader title="Upload / Intake" subtitle="Batch upload assets and assign metadata.">
        ...
        <PillButton type="submit" tone="primary" disabled={!canSend || submitting}><Upload size={16} />{submitting ? "Submitting..." : canSend ? "Start upload" : "Contributor access required"} <ChevronDown size={14} /></PillButton>
      </PageHeader>
      <div className="proto-upload-workspace">
        <main>
          ...
          <section className="proto-panel proto-upload-queue">
            ...
            {queueRows.map(({ file, status, progress }, index) => <div className="proto-upload-row" key={`${file.name}-${index}`}><span>{assets[index] ? <AssetImage asset={assets[index]} /> : <FileText size={18} />}</span><strong>{file.name}</strong><small>{formatBytes(file.size)}</small><UploadStatusChip status={status} /><div className="proto-upload-progress"><ProgressMeter value={progress} /><small>{progress}%</small></div><button type="button" aria-label={files.length ? `Remove ${file.name}` : `${status === "Queued" ? "Cancel" : "Pause"} ${file.name}`} onClick={() => files.length ? setFiles((current) => current.filter((_, fileIndex) => fileIndex !== index)) : toast.message("Select real files to edit the intake queue.")}>{files.length || status === "Queued" ? <X size={14} /> : <Pause size={14} />}</button></div>)}
          ...
          <div className="proto-upload-bottom">
            ...
            <section className="proto-panel"><h2>Batch summary</h2><dl className="proto-dl"><div><dt>Total files</dt><dd>{queueFiles.length}</dd></div><div><dt>Uploaded</dt><dd>{queueRows.filter((row) => row.status === "Completed").length}</dd></div><div><dt>Queued</dt><dd>{queueRows.filter((row) => row.status === "Queued").length}</dd></div><div><dt>Failed</dt><dd>{queueRows.filter((row) => row.status === "Failed").length}</dd></div><div><dt>Total size</dt><dd>{formatBytes(queueFiles.reduce((sum, file) => sum + file.size, 0))}</dd></div></dl></section>
          </div>
        </main>
  ...
  return (
    <section className="proto-page proto-requests-page">
      <div className="proto-requests-grid">
        ...
        {selected ? <aside className="proto-request-panel"><div className="proto-inspector-nav"><ChevronLeft size={15} /><span>1 of 48</span><ChevronRight size={15} /><X size={15} /></div><header><h2>{selected.request}</h2><StatusChip label={selected.status} tone="review" /><p>Requested by {selected.requester} on May 8, 2024</p></header><SegmentedTabs tabs={["Details", "Comments", "Activity"]} active={panelTab} onChange={setPanelTab} />{panelTab === "Details" ? <><section><h3>Request overview</h3><dl className="proto-dl"><div><dt>Due date</dt><dd>{selected.due}</dd></div><div><dt>Priority</dt><dd>{selected.priority}</dd></div><div><dt>Requested usage</dt><dd>{selected.usage}</dd></div></dl><p>We need hero images, lifestyle shots, and product close-ups for this campaign across web, social, and print.</p></section><section><h3>Approval state</h3><div className="proto-stepper"><span className="is-done">Requested</span><span className="is-active">In Review</span><span>Approved</span><span>Fulfilled</span></div></section><section><h3>Requested files</h3><div className="proto-request-files">{(requestAssets.data?.assets || []).slice(0, 6).map((asset) => <span key={asset.id}><AssetImage asset={asset} /></span>)}</div><PillButton onClick={() => setMessage("Requested files use role-safe media records. Request fulfillment stays queued until durable workflow storage exists.")}>View all files</PillButton></section><section><h3>Fulfillment</h3><label className="proto-field"><span>Owner</span><input readOnly value="Taylor Morgan" /></label><label className="proto-field"><span>Due date</span><input readOnly value={selected.due} /></label><label className="proto-field"><span>Priority</span><input readOnly value={selected.priority} /></label></section></> : <section><h3>{panelTab}</h3><p>{panelTab === "Comments" ? "Comments will persist after durable request workflow storage is connected. No fake request update is recorded in local beta." : "Activity is read-only for local beta request examples."}</p></section>}<footer><PillButton onClick={() => setMessage("Request status update is queued/truthful only after durable request workflow storage is connected. No status changed.")}>Update status</PillButton><PillButton tone="primary" onClick={() => setMessage("Fulfillment is disabled in local beta; no fake completion was recorded.")}>Mark as fulfilled</PillButton></footer>{message ? <p className="proto-gate-note">{message}</p> : null}</aside> : <aside className="proto-request-panel"><p>No requests match this filter.</p></aside>}
      </div>
    </section>
  );
}
