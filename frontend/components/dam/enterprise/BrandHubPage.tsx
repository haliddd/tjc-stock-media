...
import { AlertTriangle, CheckCircle2, Database, FileText, MoreHorizontal, PackageCheck, Share2, ShieldAlert, Star } from "lucide-react";
...
export function EnterpriseBrandHubPage() {
  ...
  const shareDisabledReason = governance?.canShare ? undefined : governance?.summary || "Handoff waits for configured Brand Kit assets that pass current role policy.";
  const deliveryDisabledReason = governance?.canDownloadKit ? undefined : governance?.summary || (role === "DAM Admin" ? "Delivery check disabled until this kit is connected to a ResourceSpace collection." : "Delivery check disabled until this kit is connected to approved media.");
  ...
  const configureCollection = () => {
    ...
    setKitMessage(role === "DAM Admin"
      ...
      : "Ask a DAM Admin to connect the approved media set.");
  };
  const focusShare = () => {
    document.getElementById("brand-kit-share")?.scrollIntoView({ behavior: "smooth", block: "center" });
    setKitMessage(governance?.deliveryReady ? "Readiness packet is prepared. Invite delivery is still off." : "Handoff waits for approved kit assets.");
  };
  return (
    <div className="enterprise-page enterprise-brand">
      <div className="ed-brand-top"><div className="ed-breadcrumb">Brand Hub <span>›</span> Ministry Kits <span>›</span> MVP 2024</div><div><ActionButton icon={Star} onClick={() => setKitMessage("MVP 2024 saved as a local preference.")}>Save</ActionButton><ActionButton icon={Share2} disabled={Boolean(governance && !governance.canShare)} disabledReason={shareDisabledReason} onClick={focusShare}>Prepare handoff</ActionButton><ActionButton tone="dark" icon={ShieldAlert} disabled={Boolean(governance && !governance.canDownloadKit)} disabledReason={deliveryDisabledReason} onClick={() => setKitMessage(governance?.deliveryReady ? "Kit readiness packet is prepared. No ZIP, public link, invite, or download was created." : deliveryDisabledReason || "Delivery check disabled.")}>Check delivery</ActionButton><div className="ed-action-menu-wrap"><IconButton label="More" onClick={() => setMoreOpen((open) => !open)}><MoreHorizontal size={16} /></IconButton>{moreOpen ? <div className="ed-more-actions-menu ed-brand-more" role="menu"><button type="button" onClick={configureCollection}>View setup details<span>{role === "DAM Admin" ? "Show server env key and collection status." : "Admin setup required."}</span></button><button type="button" onClick={() => { navigator.clipboard?.writeText(window.location.href); setKitMessage("Kit page link copied."); setMoreOpen(false); }}>Copy page link<span>Copies this Brand Hub URL.</span></button><a href={routeWithRole("/help", role)} onClick={() => setMoreOpen(false)}>Open Help Center<span>Role-safe media guidance.</span></a></div> : null}</div></div></div>
      ...
      <div className="ed-brand-layout">
        ...
        <aside className="ed-brand-rail"><section id="brand-kit-share" className="ed-card"><h3>Handoff draft</h3><p>Prepare an internal handoff note for MVP 2024. No email or link is sent.</p><input className="ed-input" value={invite} onChange={(event) => setInvite(event.target.value)} placeholder="Enter email address" aria-label="Invite email address" /><ActionButton tone="primary" disabled={!invite.trim()} onClick={() => { setSentInvite(invite); setInvite(""); setKitMessage("Invite draft prepared. No email was sent."); }}>Prepare invite draft</ActionButton>{sentInvite ? <p className="ed-inline-success">Invite draft prepared for {sentInvite}</p> : null}</section><section className="ed-card"><h3>Kit details</h3><dl className="ed-metadata">{[["Current section", section], ["Collection", role === "DAM Admin" && connected ? String(kit?.resourceSpaceCollectionId || "Configured") : connected ? "Configured" : "Not connected"], ["Config key", role === "DAM Admin" ? kit?.collectionEnvKey || "BRAND_KIT_MVP_2024_COLLECTION_ID" : "Admin only"], ["Delivery", connected ? `${kitAssets.length} governed ${role === "DAM Admin" ? noun : "kit"} references` : "Disabled"], ["Owner", kit?.owner || "Brand Team"]].map(([l, v]) => <div key={l}><dt>{l}</dt><dd>{v}</dd></div>)}</dl></section><section className="ed-card"><header className="ed-card-head"><h3>Mapped references</h3></header>{connected && kitAssets.length ? kitAssets.slice(0, 5).map((asset) => <p className="ed-file-row" key={asset.id}><FileText size={17} />{displayTitle(asset)}<small>{role === "DAM Admin" ? `${recordLabel} ${assetRecordRef(asset)}` : "Media reference"}</small></p>) : <p>{role === "DAM Admin" ? `Connect this kit to a ${noun} collection.` : "Connect this kit to approved media."}</p>}</section></aside>
      </div>
    </div>
  );
}
