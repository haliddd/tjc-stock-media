...
export function EnterpriseBrandHubPage() {
  ...
  const gateDisabledReason = governance?.canDownloadKit ? undefined : governance?.summary || (role === "DAM Admin" ? "Gate check disabled until this kit is connected to a ResourceSpace collection." : "Gate check disabled until this kit is connected to approved media.");
  ...
  const focusReviewNote = () => {
    ...
    setKitMessage(governance?.deliveryReady ? "Review note can be drafted. Email sending and file packaging stay off." : "Review note waits for approved kit assets.");
  };
  return (
    <div className="enterprise-page enterprise-brand">
      ...
      <div className="ed-brand-layout">
        <main className="ed-brand-main">
          ...
          <section id="brand-overview" className="ed-brand-hero"><div><span>MINISTRY KIT</span><h1>Brand Hub <em>{connected ? "Connected" : "Setup needed"}</em></h1><p><strong>{kit?.title || "MVP 2024"}.</strong> Editorial guidance can be curated here, but file access waits for approved media records.</p><dl><div><dt>Owner</dt><dd>{kit?.owner || "Brand Team"}</dd></div><div><dt>Data source</dt><dd>{role === "DAM Admin" ? brandSourceLabel : connected ? "Media library" : "Not connected"}</dd></div><div><dt>Next step</dt><dd>{connected ? "Review mapped assets" : "Connect media set"}</dd></div></dl></div></section>
          {brandKit.loading ? <LoadingCard label="Loading Brand Kit mapping..." /> : null}
          {!brandKit.loading && !connected ? <section className="ed-card ed-empty-state"><Database size={24} /><h2>{role === "DAM Admin" ? `Connect this kit to a ${noun} collection` : "Connect this kit to approved media"}</h2><p>{role === "DAM Admin" ? <>Set <strong>{kit?.collectionEnvKey || "BRAND_KIT_MVP_2024_COLLECTION_ID"}</strong> before showing governed kit references.</> : "Ask a DAM Admin to connect approved kit assets."} Review-note and gate-check actions stay disabled until setup is complete.</p><ActionButton onClick={configureCollection}>{role === "DAM Admin" ? "View setup details" : "Ask DAM Admin"}</ActionButton></section> : null}
