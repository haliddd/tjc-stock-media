...
describe("request record role payloads", () => {
  it("scopes non-reviewer request lists to the requesting identity", () => {
    ...
    expect(requestRecordsForIdentityPayload({ id: "sso:viewer@example.test", role: "Viewer" }, [mine, anotherViewer]))
      .toHaveLength(1);
    expect(requestRecordsForIdentityPayload({ id: "sso:viewer@example.test", role: "Viewer" }, [mine, anotherViewer])[0]?.id)
      .toBe("req-mine");
  });

  it("keeps requester-owned DAM review records visible even when role fit is reviewer-only", () => {
    const mine = record({
      id: "req-review-mine",
      type: "DAM review",
      requestedBy: "sso:viewer@example.test",
    ...
    const anotherViewer = record({
      id: "req-review-other",
      type: "DAM review",
      requestedBy: "sso:other@example.test",
      roleFit: ["Contributor", "Reviewer", "DAM Admin"]
    });
    ...
    expect(payload[0]?.id).toBe("req-review-mine");
  });
  ...
  it("keeps reviewer queue broad while preserving internal ids for reviewers", () => {
    ...
    expect(payload[0]?.resourceSpaceId).toBe("rs-123");
    expect(payload[0]?.linkedPendingWriteId).toBe("pending-1");
    expect(payload[0]?.storageMode).toBe("local-json");
  });
